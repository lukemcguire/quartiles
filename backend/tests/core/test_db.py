"""Tests for database initialization."""

from sqlmodel import Session, select

from app.core.config import settings
from app.core.db import init_db
from app.models import User


def test_init_db_creates_superuser(session: Session) -> None:
    """Test superuser creation when none exists."""
    # Clear ALL existing superusers (handles parallel test execution)
    existing_superusers = session.exec(select(User).where(User.is_superuser)).all()
    for user in existing_superusers:
        session.delete(user)
    session.commit()

    # Initialize DB should create superuser
    init_db(session)

    user = session.exec(select(User).where(User.is_superuser)).first()
    assert user is not None
    assert user.email == settings.FIRST_SUPERUSER


def test_init_db_skips_existing_superuser(session: Session) -> None:
    """Test no duplicate superuser created."""
    from sqlalchemy import func

    # First init
    init_db(session)
    count_before = session.exec(select(func.count()).select_from(User).where(User.is_superuser)).one()

    # Second init should not create another superuser
    init_db(session)
    count_after = session.exec(select(func.count()).select_from(User).where(User.is_superuser)).one()

    assert count_before == count_after
